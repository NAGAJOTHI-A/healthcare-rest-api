...
public class PatientServiceImpl implements PatientService{
	...
	@Override
	public Map<String, Object> getDoctors(String name, String specialization, int size, int page, String sort,
			boolean desc) {
		List<Doctor> doctors=null;
		
		if(name==null && specialization==null) 
			doctors=doctorDao.getAllDoctors(size,page,sort,desc);
		else {
		  	if(name !=null && specialization!=null) 
	...
	public Map<String, Object> getDoctorsTimeSlot(Long id) {
		...
		return Map.of("message","Doctor Available at Below Time Slots","timeSlots",userMapper.toDoctorTimeSlotDtoList(timeSlots));
	}

	public Map<String,Object> bookAppointment(Long id,String email)
	{
		Patient patient=patientDao.findPatientByEmail(email);
		DoctorTimeSlot doctorTimeSlot=doctorDao.getDoctorsTimeSlotById(id);
		Appointment appointment=new Appointment(null,doctorTimeSlot.getTimeSlot(),doctorTimeSlot.getDoctor(),patient,false);
